    ...
    def generate_region_masks(self, video_file_path, annotations_file_path):
        try:
            ...
            if not os.path.exists(video_file_path):
                raise FileNotFoundError('Quitting: Video file path does not exist')

            # Checking if annotations file path exist
            if not os.path.exists(annotations_file_path):
                raise FileNotFoundError('Quitting: Annotations file path does not exist')
            ...
            progress_bar = tqdm(total=5, unit=' mask')
            region_masks['green'][region_masks['green'] >= (num_observations // 4)] = 255
