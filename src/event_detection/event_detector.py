    ...
    def detect_motion(self, annotations_file_path):
        try:
            ...
            if not os.path.exists(annotations_file_path):
                raise FileNotFoundError('Annotations file path does not exist')

            # Reading object annotations
            ...
            for key in coin_coordinates.keys():
                print('Detecting events for: {} ...'.format(key))
            ...
            # Writing detected events to file
...
# Driver Code
# detector = Detector()
# annotations_file_path = os.path.join(paths.data_folder_path, 'detected_and_tracked_objects', '77_bonuss.json')
